...
export default function QuickResponseManager({ responses, onSend, onSave, onDelete, onClose }: Props) {
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 10 }}
      className="absolute bottom-16 left-0 right-0 mx-3 bg-black/95 backdrop-blur-xl border border-white/10 rounded-2xl p-3 shadow-2xl z-50 max-h-[400px] flex flex-col">
      {/* Header */}
            ...
            <div className="flex gap-1 flex-wrap">
              {['all', ...CATEGORIES].map(c => (
                <button key={c} onClick={() => setFilterCat(c)}
                  className={`px-2 py-0.5 rounded-md text-[8px] font-bold uppercase tracking-wider transition-all ${filterCat === c ? 'bg-brand-purple/20 text-brand-purple' : 'bg-white/5 text-muted/50 hover:text-muted'}`}>
                  {c === 'all' ? 'Todas' : c}
                </button>
              ))}
            </div>
              ...
              {filtered.map(qr => (
                <div key={qr.id} className="group relative text-left p-2.5 rounded-xl bg-white/5 hover:bg-brand-purple/10 border border-white/5 hover:border-brand-purple/20 transition-all">
                  ...
                  <div className="absolute top-1 right-1 hidden group-hover:flex gap-0.5">
                    ...
                    <button onClick={() => startEdit(qr)} className="p-0.5 rounded bg-white/10 hover:bg-white/20"><Edit2 size={9} className="text-blue-400" /></button>
                    <button onClick={() => onDelete(qr.id)} className="p-0.5 rounded bg-white/10 hover:bg-white/20"><Trash2 size={9} className="text-red-400" /></button>
                  </div>
                </div>
              ))}
            </div>
          ...
          <motion.div key="edit" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="space-y-2">
            <div className="flex gap-2">
              <input value={editing.label || ''} onChange={e => setEditing({ ...editing, label: e.target.value })}
                placeholder="Etiqueta" className="flex-1 px-3 py-1.5 rounded-lg text-[11px] bg-white/5 border border-white/10 focus:border-brand-purple/30 outline-none" />
              <select value={editing.category || 'general'} onChange={e => setEditing({ ...editing, category: e.target.value })}
                className="px-2 py-1.5 rounded-lg text-[10px] bg-white/5 border border-white/10 outline-none">
            ...
            <div className="flex justify-end gap-2">
              <button onClick={() => { setMode('list'); setEditing(null); }}
                className="px-3 py-1 rounded-lg text-[10px] font-bold text-muted hover:bg-white/5">Cancelar</button>
